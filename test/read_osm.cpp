...
int main(){
    ...
    bk::block car;
    car.set_block(100,100);
    car.set_map(LAT_B-0.000910, LNG_B-0.000570);//00285);//405
    car.set_angle(angle);
    car.set_offset(offsetMapVisualizer);
    car.set_carP(100, 100);
    car.set_block_point();
    car.set_scale(8);
    int visible = 255;
    while(key!=27){
        cv::Mat mapVisualizer = cv::Mat(cv::Size(200,200), CV_8UC1, cv::Scalar(255));
